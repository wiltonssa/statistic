...
for(i in 1:M){
  ...
  dados[i, "upper"] = mean(amostra) + qt(0.975, n-1) * sd(amostra)/n^0.5
  dados[i, "lower"] = mean(amostra) - qt(0.975, n-1) * sd(amostra)/n^0.5
}

# determino se o intervalo contem ou nao a media
# TRUE = Contem a media
# FALSE = Não contem a media
dados$HasTheMean = dados$upper >= pop.Media &  dados$lower <= pop.Media
...
grafico = ggplot(dados) +
  ...
  scale_color_manual("Contem a média",
                     ...
                     breaks = c("TRUE", "FALSE"),
                     labels = c("Sim", "Não")) +
  labs(title = "Intervalo de confiança da Média",
